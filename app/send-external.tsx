...
export default function SendExternalScreen() {
  ...
  // Sender enters amount in their own wallet currency
  const senderCurrency = user?.wallet?.currency || "INR";
  const senderSymbol =
    senderCurrency === "USD"
      ? "$"
      : senderCurrency === "EUR"
  ...
  const handleReview = () => {
    if (amount === "0") return;
    router.push({
      pathname: "/external-payment-confirm",
      params: {
        fiatAmount: amount,
        senderCurrency,
        receiverCurrency,
        ...
        <SafeAreaView className="flex-1 flex-col justify-between">
          <View>
            ...
            <View className="items-center mt-2 px-6">
              <View className="bg-zinc-900/80 border border-zinc-800 rounded-full flex-row items-center px-4 py-2">
                ...
                <View>
                  ...
                  <Text className="text-zinc-600 text-[10px] font-myMedium">
                    via {methodLabel} • {receiverCurrency}
                  </Text>
                </View>
              </View>
            </View>
          ...
          <View className="flex-1 justify-center items-center">
            ...
            {amount !== "0" && (
              <Text className="text-zinc-600 text-xs font-myMedium mt-1">
                Recipient receives in {receiverCurrency} •{" "}
                {methodEta.toLowerCase()}
              </Text>
            )}
          </View>
